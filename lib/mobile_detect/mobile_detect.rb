class MobileDetect
  ...
  def is_mobile_regexp
    regexp_device(PhoneDevices)
  end
  ...
  def is_mobile_headers
    ...
    if @req_headers['HTTP_X_WAP_PROFILE'] ||
        ...
        @req_headers['HTTP_PROFILE'] ||
        @req_headers['HTTP_X_OPERAMINI_PHONE_UA'] || # Reported by Nokia devices (eg. C3)
        @req_headers['HTTP_X_NOKIA_IPADDRESS'] ||
        @req_headers['HTTP_X_NOKIA_GATEWAY_ID'] ||
        @req_headers['HTTP_X_ORANGE_ID'] ||
        @req_headers['HTTP_X_VODAFONE_3GPDPCONTEXT'] ||
        @req_headers['HTTP_X_HUAWEI_USERID'] ||
